  ...
  socket.on("ReadRows", async function (ClientData) {
    ...
    tabletLogs.push({
      message:
        "client => id: " +
        socket.client.id +
        " requested data reterival from Tablet Server 1",
      timeStamp: Date.now(),
    });
  ...
  socket.on("Set", async function (ClientData) {
    ...
    tabletLogs.push({
      message:
        "client => id: " +
        socket.client.id +
        " requested data update from Tablet Server " +
        tabletNumber,
      timeStamp: Date.now(),
    });
    ...
    tabletLogs.push({
      message:
        "client => id: " +
        socket.client.id +
        " acquired the lock from Tablet Server " +
        tabletNumber,
      timeStamp: Date.now(),
    });
    try {
      ...
      tabletLogs.push({
        message:
          "client => id: " +
          socket.client.id +
          " released the lock from Tablet Server " +
          tabletNumber,
        timeStamp: Date.now(),
      });
    }
      ...
      if (!data.data) {
        ...
        tabletLogs.push({
          message:
            "client => id: " +
            socket.client.id +
            " requested Delete Cells from Tablet Server " +
            tabletNumber +
            "  => Error: " +
            data.err,
          timeStamp: Date.now(),
  ...
  socket.on("DeleteRow", async function (ClientData) {
        ...
        if (data1.ids.length == 0) {
          ...
          tabletLogs.push({
            message:
              "client => id: " +
              socket.client.id +
              " requested Delete Row from Tablet Server 1  => Error: No Anime Was Deleted",
            timeStamp: Date.now(),
          });
        } else {
    ...
    if (ids.ids2.length != 0) {
      ...
      tabletLogs.push({
        message:
          "client => id: " +
          socket.client.id +
          " acquired the lock on Tablet server 1 tablet 2",
        timeStamp: Date.now(),
      });
      try {
        ...
        if (data2.ids.length == 0) {
          ...
          tabletLogs.push({
            message:
              "client => id: " +
              socket.client.id +
              " requested Delete Row to master from Tablet Server 1 Tablet 2 => Succeeded",
            timeStamp: Date.now(),
          });
        }
      } finally {
    ...
    if(ids.ids1.length==0 && ids.ids2.length ==0){
      ...
      tabletLogs.push({
        message:
          "client => id: " +
          socket.client.id +
          " requested Delete Row to client from Tablet Server 1  => Error : No Key Rows Exists Tablet Server 1",
        timeStamp: Date.now(),
      });
      return;
    }
    ...
    tabletLogs.push({
      message:
        "client => id: " +
        socket.client.id +
        " requested Delete Row to client from Tablet Server 1  => Succeeded",
      timeStamp: Date.now(),
    });
